...
const  storeManagementService = {
    ...
    async setUpStore(store: Store) {
        try {
            ...
            const {data:vendorStore} = await storeManagementService.createStore(store);
            ...
            const [{data:storeTradeManager}, {data:storeWallet}, {data:storeCollection}] = await Promise.all([
                tradeManagementService.createStoreTradeManager(tradeManager),
                storeManagementService.createStoreWallet(vendorId, storeId),
                storeManagementService.createStoreCollection(defaultStoreCollection)
            ])
            // update vendor store with collection and trade manager in background
            storeManagementService.updateStore(
    ...
    async updateStoreType(
        ...
        try {
            ...
            if (!await storeManagementService.getStore(storeId)) {
                return ({
                    success: false,
                    status: 400,
